...
export function showSkeleton(containerId, type) {
  ...
  if (type === "keys") {
    for (let i = 0; i < 3; i++) {
      ...
      skeleton.className =
        "flex flex-col md:flex-row md:items-center justify-between gap-3 p-6 rounded-3xl border border-white/5 bg-zinc-900/30";
    ...
    for (let i = 0; i < 2; i++) {
      const skeleton = document.createElement("div");
      skeleton.className =
        "flex items-center justify-between p-3 rounded-lg bg-black/40 border border-white/5";
      skeleton.style.animationDelay = `${i * 0.05}s`;
      skeleton.innerHTML = `
    ...
    for (let i = 0; i < 5; i++) {
      ...
      row.className =
        "flex items-center justify-between p-4 mb-3 rounded-xl bg-black/20 border border-white/5";
...
export function showStatSkeletons() {
  const statElements = [
    ...
    dom.totalDomainsEl,
    dom.activeDomainsEl,
    dom.inactiveDomainsEl,
    dom.domainsTotalEl,
    dom.domainsActiveEl,
    dom.domainsInactiveEl,
    dom.statTotalRequests,
    dom.statTotalTokens,
    dom.statAvgResponse,
    dom.statSuccessRate,
  ];
  ...
  if (dom.domainLimitBadge) {
    ...
    dom.domainLimitBadge.innerHTML =
      '<div class="w-16 h-4 bg-primary/20 rounded animate-pulse inline-block"></div>';
  }
  if (dom.usageNumber) {
  ...
  if (dom.usageLimit) {
    ...
    dom.usageLimit.innerHTML =
      '<div class="w-10 h-4 bg-white/10 rounded animate-pulse inline-block"></div>';
  }
  if (dom.usagePercentText) {
    dom.usagePercentText.style.opacity = "1";
    dom.usagePercentText.innerHTML =
      '<div class="w-12 h-3 bg-white/10 rounded animate-pulse"></div>';
  }
  if (dom.usageRemaining) {
    dom.usageRemaining.style.opacity = "1";
    dom.usageRemaining.innerHTML =
      '<div class="w-8 h-3 bg-white/10 rounded animate-pulse inline-block"></div>';
  }
}
...
export function showUsageSkeletons() {
  if (dom.statRequestsTrend) {
    ...
    dom.statRequestsTrend.innerHTML =
      '<div class="w-20 h-3 bg-white/10 rounded animate-pulse"></div>';
  }
  if (dom.statTokensTrend) {
    dom.statTokensTrend.style.opacity = "1";
    dom.statTokensTrend.innerHTML =
      '<div class="w-20 h-3 bg-white/10 rounded animate-pulse"></div>';
  }
    ...
    if (canvas && canvas.parentElement) {
      ...
      if (!placeholder) {
        ...
        placeholder.className =
          "chart-skeleton w-full h-full bg-white/5 animate-pulse rounded-xl absolute inset-0";
    ...
    state.turnstileWidgetId = turnstile.render(container, {
      ...
      callback: function (token) {
        ...
        if (containerId === "turnstile-container") {
          if (dom.saveKeyBtn) dom.saveKeyBtn.classList.remove("hidden");
        } else if (containerId === "captchaModalContainer") {
          if (dom.captchaModalConfirm) dom.captchaModalConfirm.classList.remove("hidden");
        }
      ...
      "expired-callback": function () {
        ...
        if (containerId === "turnstile-container") {
          if (dom.saveKeyBtn) dom.saveKeyBtn.classList.add("hidden");
        } else if (containerId === "captchaModalContainer") {
          if (dom.captchaModalConfirm) dom.captchaModalConfirm.classList.add("hidden");
        }
      },
...
export function showCaptchaModal(title, desc, action, data = null) {
  if (dom.captchaModalTitle) dom.captchaModalTitle.textContent = title;
  if (dom.captchaModalDesc) dom.captchaModalDesc.textContent = desc;
  state.pendingAction = action;
  state.pendingActionData = data;
  if (dom.captchaModal) dom.captchaModal.classList.remove("hidden");
  if (dom.captchaModalConfirm) dom.captchaModalConfirm.classList.add("hidden");
  setTimeout(() => renderTurnstile("captchaModalContainer"), 200);
}

export function closeCaptchaModal() {
  if (dom.captchaModal) dom.captchaModal.classList.add("hidden");
  resetTurnstile();
...
export function updateUserUI(user) {
  ...
  if (dom.sidebarAvatar)
    dom.sidebarAvatar.src =
      user.photoURL || "https://ui-avatars.com/api/?name=User&background=a855f7&color=fff&size=40";
  ...
  // Welcome (only in dashboard)
  if (dom.welcomeMessageEl)
    dom.welcomeMessageEl.textContent = `Welcome back, ${displayName}!`;

  // Settings profile (only in settings page)
  if (dom.settingsAvatar)
    dom.settingsAvatar.src =
      user.photoURL || "https://ui-avatars.com/api/?name=User&background=a855f7&color=fff&size=80";
  ...
  // Social auth handling (for settings page)
  ...
  // Ensure delete modal is hidden (in case it was left open)
