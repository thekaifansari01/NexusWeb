...
function getConfig() {
    return {
        ...
        scriptUrl: scriptUrl.value.trim() || 'https://cdn.jsdelivr.net/npm/nexus-web-assistant@4.1.0/dist/nexus-assistant.min.js',
...
function generateScript(config) {
    ...
    lines.push(`  };`);
    lines.push(`<\/script>`);
    lines.push(`<script defer src="${config.scriptUrl}"><\/script>`);
    return lines.join('\n');
}
...
<\/script>
<script defer src="${config.scriptUrl}"><\/script>
</body>
</html>`;
...
function initListeners() {
    [scriptUrl, botName, greeting, themeSelect, modelSelect, systemPrompt].forEach(el => {
